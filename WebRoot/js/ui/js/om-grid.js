    ...
    $.omWidget('om.omGrid', {
        ...
        _fixHeaderWidth:function(autoExpandColIndex , allColsWidth){
        	...
        	if(autoExpandColIndex != -1){ //说明有某列要自动扩充
                ...
                var tableWidth = $grid.width(),//mod by umpay lizhen，原来是var tableWidth = $grid.width()-22，导致了表格右侧出现多余的空白
        ...
        _buildPagingToolBar:function(){
           ...
           //add by yangwr start
                '<div class="btnseparator"></div>'+
				'<div class="pGroup"><span class="pPageSize">每页显示'+
					'<select>'+ 
				...
				'条</span></div>'+
            	'</div>');
				$('.pPageSize select', pDiv).change(function(e) {
					self._changePageSize();
				});
			//add by yangwr end	
            ...
            $('.pControl input', pDiv).keydown(function(e) {
                if (e.keyCode == $.om.keyCode.ENTER) {
					...
					return false;//add by yangwr 阻止事件
				}
            });
        ...
        //add by yangwr start
        _changePageSize : function() {//change page size
			if (this.loading) {
				return true;
			}
			var iPageSize = parseInt($('.pPageSize select',this.element.closest('.om-grid')).val());
			this.options.limit = iPageSize;
...
//                bodyContent.push("<tr _grid_row_id="+(self._guid++)+" class='om-grid-row " + rowCls + "'>");
            	...
            	//add by umpay lizhen start，保留tr的斑马纹样式
            	var oddAndEvenClasses=['oddRow','evenRow'];
            	var rowCls = oddAndEvenClasses[i % 2];
                var rowValues=self._buildRowCellValues(colModel,rowData,i);
                bodyContent.push("<tr _grid_row_id="+(self._guid++)+" class='om-grid-row " + rowCls + "'>");
                //add by umpay lizhen end
               	...
               	$(gridHeaderCols).each(function(index){
                    ...
                    if(axis == 'indexCol'){
                    	...
                    	//add by umpay lizhen start，在checkbox的位置显示自定义样式
                    	if(isRowClassesFn)
                    		html = '<span class="'+rowClasses(i,rowData)+'"/>';
                    	else
                    		html = '<span class="checkbox"/>';
                    	//add by umpay lizhen end
