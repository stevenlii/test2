...
<head>
    ...
    <script type="text/javascript" src="${pageContext.request.contextPath}/js/ui/js/om-messagebox.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath}/js/ui/js/om-button.js"></script>
    <!-- view_source_begin
    <style>
...
<!-- view_source_begin -->
    ...
    <!-- view_source_end -->
    <div id="view-desc">
        $.omMessageBox.alert(config)可以通过type属性来定义几种常见的提示图标。<br/>
         如果要自定义图标时只需要将type设成任意字符串（如"<font color='red'>xyz</font>"），然后再在页面head中添加样式　　<span style="font-weight: bolder">.om-messageBox-image-<font color='red'>xyz</font>{background-image:url(图片路径);}　　</span>即可。
    </div>
</body>
